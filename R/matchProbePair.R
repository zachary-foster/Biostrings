...
setGeneric("matchProbePair", signature="subject",
    function(Fprobe, Rprobe, subject,
             max.mismatch=0, min.mismatch=0, with.indels=FALSE, fixed=TRUE, algorithm="auto",
...
setMethod("matchProbePair", "DNAString", 
             ...
             logfile=NULL, verbose=FALSE)
    {
        ...
        Fp_hits <- start(matchPattern(F, subject,
                                      max.mismatch=max.mismatch, min.mismatch=min.mismatch,
                                      with.indels=with.indels, fixed=fixed, algorithm=algorithm))
        Rp_hits <- start(matchPattern(R, subject,
                                      max.mismatch=max.mismatch, min.mismatch=min.mismatch,
                                      with.indels=with.indels, fixed=fixed, algorithm=algorithm))

        ## F and R hits on the - strand
        Fm_hits <- end(matchPattern(reverseComplement(F), subject,
                                    max.mismatch=max.mismatch, min.mismatch=min.mismatch,
                                    with.indels=with.indels, fixed=fixed, algorithm=algorithm))
        Rm_hits <- end(matchPattern(reverseComplement(R), subject,
                                    max.mismatch=max.mismatch, min.mismatch=min.mismatch,
                                    with.indels=with.indels, fixed=fixed, algorithm=algorithm))
